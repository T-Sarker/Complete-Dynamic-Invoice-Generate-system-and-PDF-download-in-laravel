...
@section('content')
    <div class="container">
      <div class="my-3">
        ...
        <form class="mt-5" method="POST" action="{{route('company.update')}}" >
          @csrf
          ...
          <div class="mb-3">
            <label for="phone" class="form-label">Phone</label>
             <input type="tel" class="form-control" id="phone" name="phone" placeholder="Company phone" value="{{$company->phone}}">
              @error("phone")
                <p class="alert alert-danger">{{$message}}</p>
              @enderror
          </div>          
